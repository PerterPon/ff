...
describe('Exchange - 挂单交易测试', () => {
    ...
    beforeEach(() => {
        ...
        exchange = new exchange_1.Exchange(wallet, {
            spotBuy: test_utils_1.FEE_RATE.spotBuy,
            spotSell: test_utils_1.FEE_RATE.spotSell,
            limitBuy: test_utils_1.FEE_RATE.limitBuy,
            limitSell: test_utils_1.FEE_RATE.limitSell,
            shortOpen: test_utils_1.FEE_RATE.shortOpen,
            shortClose: test_utils_1.FEE_RATE.shortClose
        });
        jest.clearAllMocks();
    ...
    describe('复杂挂单场景', () => {
        ...
        test('同时存在多个买卖单', () => {
            ...
            const spotPrice = 40000;
            (0, test_utils_1.setMockPrice)(main_1.Symbol.BTC_USDT, spotPrice);
            exchange.spotBuy(main_1.Symbol.BTC_USDT, initialAmount);
            exchange.onPriceUpdate(main_1.Symbol.BTC_USDT, spotPrice);
            // 买入后总资产
            // 总资产 = 可用余额 (5996) + BTC 价值 (0.1 * 40000) = 9996
            const spotBuyFee = (0, test_utils_1.calculateFee)(initialAmount, spotPrice, test_utils_1.FEE_RATE.spotBuy);
            const expectedAfterBuy = INITIAL_BALANCE - spotBuyFee;
            expect(exchange.getTotalAssetValue()).toBeCloseTo(expectedAfterBuy, 1);
            // 2. 挂买卖单
            ...
            // 先挂卖单
            const sellOrderId = exchange.placeSellOrder(main_1.Symbol.BTC_USDT, sellPrice, orderAmount);
            // 再挂买单
            const buyOrderId = exchange.placeBuyOrder(main_1.Symbol.BTC_USDT, orderAmount, buyPrice);
            // 挂单后总资产
            // 总资产 = 可用余额 (4093.8) + 
            //          剩余 BTC 价值 (0.05 * 40000) + 
            //          冻结 BTC 价值 (0.05 * 40000) +
            //          冻结买单资金 (0.05 * 38000 + 3.8) = 9996
            const buyFee = (0, test_utils_1.calculateFee)(orderAmount, buyPrice, test_utils_1.FEE_RATE.limitBuy);
            ...
            const sellFee = (0, test_utils_1.calculateFee)(orderAmount, sellPrice, test_utils_1.FEE_RATE.limitSell);
