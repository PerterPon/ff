...
describe('Wallet 类测试', () => {
    let wallet;
    beforeEach(() => {
        wallet = new wallet_1.Wallet(1000); // 初始法币余额 1000
        jest.clearAllMocks();
    });
    describe('法币相关操作', () => {
    ...
    describe('交易对余额相关操作', () => {
        ...
        test('设置交易对余额应该正常工作', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 1.5);
            expect(wallet.getBalance(main_1.Symbol.BTC_USDT)).toBe(1.5);
        });
        test('设置负数交易对余额应该抛出错误', () => {
            expect(() => wallet.setBalance(main_1.Symbol.BTC_USDT, -1)).toThrow('余额不能为负数');
        });
        test('增加交易对余额应该正常工作', () => {
            wallet.setBalance(main_1.Symbol.ETH_USDT, 2);
            wallet.addBalance(main_1.Symbol.ETH_USDT, 1);
            expect(wallet.getBalance(main_1.Symbol.ETH_USDT)).toBe(3);
        });
        test('减少交易对余额应该正常工作', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 2);
            wallet.subtractBalance(main_1.Symbol.BTC_USDT, 0.5);
            expect(wallet.getBalance(main_1.Symbol.BTC_USDT)).toBe(1.5);
        });
        test('减少超过余额的交易对余额应该抛出错误', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 1);
            expect(() => wallet.subtractBalance(main_1.Symbol.BTC_USDT, 2)).toThrow('余额不足');
        });
        test('清空特定交易对余额应该正常工作', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 1);
            wallet.clearBalance(main_1.Symbol.BTC_USDT);
            expect(wallet.getBalance(main_1.Symbol.BTC_USDT)).toBe(0);
        });
        test('清空所有交易对余额应该正常工作', () => {
            ...
            expect(wallet.getBalance(main_1.Symbol.BTC_USDT)).toBe(0);
            expect(wallet.getBalance(main_1.Symbol.ETH_USDT)).toBe(0);
        });
        test('获取所有余额应该返回正确的 Map', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 1);
            wallet.setBalance(main_1.Symbol.ETH_USDT, 2);
            const balances = wallet.getAllBalances();
            expect(balances.get(main_1.Symbol.BTC_USDT)).toBe(1);
            expect(balances.get(main_1.Symbol.ETH_USDT)).toBe(2);
        });
    });
    describe('总资产计算', () => {
        beforeEach(() => {
            price_1.getPrice.mockImplementation((symbol) => {
                const prices = {
                    [main_1.Symbol.BTC_USDT]: 40000,
                    [main_1.Symbol.ETH_USDT]: 2000
                };
                return prices[symbol];
            });
        });
        test('应该正确计算总资产（不含未实现盈亏）', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 2); // 2 * 40000 = 80000
            wallet.setBalance(main_1.Symbol.ETH_USDT, 10); // 10 * 2000 = 20000
        ...
        test('获取价格失败时应该忽略该资产', () => {
            wallet.setBalance(main_1.Symbol.BTC_USDT, 2); // 价格获取失败，忽略
            wallet.setBalance(main_1.Symbol.ETH_USDT, 10); // 10 * 2000 = 20000
