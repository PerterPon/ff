...
const dynamic_hedge_grid_1 = require("../dynamic-hedge-grid");
...
describe('DynamicHedgeGridStrategy', () => {
    ...
    beforeEach(() => {
        ...
        wallet = new wallet_1.Wallet(100000);
        exchange = new exchange_1.Exchange(wallet);
        // 创建策略实例，设置网格参数
        strategy = new dynamic_hedge_grid_1.DynamicHedgeGridStrategy({
    ...
    describe('createGrid', () => {
        it('should create grid orders with correct prices and amounts', async () => {
            await strategy['createGrid'](main_1.Symbol.BTC_USDT, currentPrice);
            ...
            expect(wallet.getBalance(main_1.Symbol.BTC_USDT)).toBeCloseTo(0, 3);
        });
        ...
        describe('createGrid edge cases', () => {
            ...
            it('should create sell orders when price is at lower limit', async () => {
                ...
                await strategy['createGrid'](main_1.Symbol.BTC_USDT, lowerLimit);
                const orders = exchange.getPendingOrders();
                // 生成预期的价格序列：从 27600 到 33000，每格 600
                const expectedPrices = Array.from({ length: gridCount }, (_, i) => lowerLimit + (i + 1) * gridSize);
            ...
            it('should create sell orders when price is below lower limit', async () => {
                ...
                await strategy['createGrid'](main_1.Symbol.BTC_USDT, belowLowerLimit);
                const orders = exchange.getPendingOrders();
                // 生成预期的价格序列：从 27000 到 33000，每格 600
                const expectedPrices = Array.from({ length: gridCount + 1 }, (_, i) => lowerLimit + i * gridSize);
            ...
            it('should create buy orders when price is at upper limit', async () => {
                ...
                await strategy['createGrid'](main_1.Symbol.BTC_USDT, upperLimit);
                const orders = exchange.getPendingOrders();
                // 生成预期的价格序列：从 32400 往下到 27000，每格 600
                const expectedPrices = Array.from({ length: gridCount }, (_, i) => upperLimit - (i + 1) * gridSize);
                ...
                expect(actualPrices).toEqual(expectedPrices);
                // 验证不需要初始买入
                const btcBalance = wallet.getBalance(main_1.Symbol.BTC_USDT);
            ...
            it('should create sell orders when price is far below lower limit', async () => {
                ...
                await strategy['createGrid'](main_1.Symbol.BTC_USDT, farBelowLimit);
                const orders = exchange.getPendingOrders();
            ...
            it('should create buy orders when price is far above upper limit', async () => {
                ...
                await strategy['createGrid'](main_1.Symbol.BTC_USDT, farAboveLimit);
                const orders = exchange.getPendingOrders();
                // 生成预期的价格序列：从 27000 到 33000
                const expectedPrices = Array.from({ length: gridCount + 1 }, (_, i) => lowerLimit + i * gridSize);
                // 验证订单数量应该是 11 个（包含所有网格点）
