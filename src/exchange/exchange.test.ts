...
describe('Exchange 类测试', () => {
    ...
    beforeEach(() => {
        ...
        jest.clearAllMocks();
        (getPrice as jest.Mock).mockReturnValue(40000);  // 设置默认价格
    });
    ...
    describe('现货交易', () => {
        describe('现货买入', () => {
            ...
            test('成功买入应该更新余额', () => {
                exchange.spotBuy(Symbol.BTC_USDT, 0.1); // 买入 0.1 BTC
            ...
            test('余额不足应该抛出错误', () => {
                // 尝试买入超过余额的数量
                expect(() => exchange.spotBuy(Symbol.BTC_USDT, 3))
                    .toThrow('法币余额不足');
        ...
        describe('现货卖出', () => {
            beforeEach(() => {
                ...
                exchange = new Exchange(wallet);
                // 先买入一些资产用于测试
                exchange.spotBuy(Symbol.BTC_USDT, 0.5);
            });

            test('成功卖出应该更新余额', () => {
                const initialAssets = exchange.getTotalAssetValue();
                exchange.spotSell(Symbol.BTC_USDT, 0.2);
                
                // 检查 BTC 减少
                expect(wallet.getBalance(Symbol.BTC_USDT)).toBe(0.3);
    ...
    describe('挂单交易', () => {
        ...
        describe('挂买单', () => {
            test('成功挂单应该冻结资金', () => {
                const orderId = exchange.placeBuyOrder(Symbol.BTC_USDT, 39000, 0.1);
            ...
            test('余额不足应该抛出错误', () => {
                ...
                expect(() => exchange.placeBuyOrder(Symbol.BTC_USDT, 40000, 3))
                    .toThrow('法币余额不足');
            });
        });
        ...
        describe('挂卖单', () => {
            ...
            test('成功挂单应该冻结资产', () => {
                const orderId = exchange.placeSellOrder(Symbol.BTC_USDT, 41000, 0.1);
                
                // 1 BTC - 0.1 BTC = 0.9 BTC
                expect(wallet.getBalance(Symbol.BTC_USDT)).toBe(0.9);
            ...
            test('余额不足应该抛出错误', () => {
                ...
                expect(() => exchange.placeSellOrder(Symbol.BTC_USDT, 41000, 1.5))
                    .toThrow('余额不足');
            });
        });
        ...
        describe('订单管理', () => {
            ...
            beforeEach(() => {
                // 确保有足够的资金和资产进行测试
                buyOrderId = exchange.placeBuyOrder(Symbol.BTC_USDT, 39000, 0.1);
                exchange.spotBuy(Symbol.BTC_USDT, 0.2);
                sellOrderId = exchange.placeSellOrder(Symbol.BTC_USDT, 41000, 0.1);
            });
            ...
            test('应该能够取消卖单', () => {
                const initialBalance = wallet.getBalance(Symbol.BTC_USDT);
                exchange.cancelOrder(sellOrderId);
                
                expect(wallet.getBalance(Symbol.BTC_USDT)).toBe(initialBalance + 0.1);
                expect(exchange.getPendingOrders()).toHaveLength(1);
            });
    ...
    describe('未实现盈亏计算', () => {
        ...
        test('价格下跌应该计算正确的未实现盈利', () => {
            ...
            // 盈利 = (40000 - 38000) * 0.1 = 200
            expect(exchange.getUnrealizedPnl(Symbol.BTC_USDT)).toBe(200);
        });
        ...
        test('不存在的仓位应该返回 0', () => {
            expect(exchange.getUnrealizedPnl(Symbol.ETH_USDT)).toBe(0);
        });
    });
    ...
    describe('未实现盈亏总和', () => {
        beforeEach(() => {
            ...
            (getPrice as jest.Mock).mockImplementation((symbol: Symbol) => {
                ...
                if (symbol === Symbol.ETH_USDT) return 2000;
                throw new Error('未知交易对');
            });

            // 开两个空仓位
            exchange.openShort(Symbol.BTC_USDT, 0.1, 2);  // 40000 价格开仓
            exchange.openShort(Symbol.ETH_USDT, 1, 2);    // 2000 价格开仓
            ...
            (getPrice as jest.Mock).mockImplementation((symbol: Symbol) => {
                if (symbol === Symbol.BTC_USDT) return 38000; // BTC 下跌，盈利 200
                if (symbol === Symbol.ETH_USDT) return 2200;  // ETH 上涨，亏损 200
                throw new Error('未知交易对');
            });
        });
        ...
        test('没有空单时应该返回 0', () => {
            ...
            // 清空所有空单
            exchange.closeShort(Symbol.BTC_USDT);
            exchange.closeShort(Symbol.ETH_USDT);
        ...
        test('价格大幅波动时应该正确计算盈亏', () => {
            // 价格剧烈变化
            (getPrice as jest.Mock).mockImplementation((symbol: Symbol) => {
                if (symbol === Symbol.BTC_USDT) return 36000; // BTC 大跌，盈利 400
                if (symbol === Symbol.ETH_USDT) return 2400;  // ETH 大涨，亏损 400
                throw new Error('未知交易对');
            });
    ...
    describe('复杂交易场景', () => {
        beforeEach(() => {
            wallet = new Wallet(10000);  // 初始余额 10000
            exchange = new Exchange(wallet);
            (getPrice as jest.Mock).mockReturnValue(100);  // BTC 初始价格 100
        });
        ...
        test('应该正确处理多个交易操作的组合', () => {
            ...
            // 4. BTC 价格上涨到 250
            (getPrice as jest.Mock).mockReturnValue(250);
            // 现货价值：1 * 250 = 250
            ...
            // 5. 卖出现货 1 BTC
            exchange.spotSell(Symbol.BTC_USDT, 1);
            // 收入：250 * 1 = 250
            // 手续费：250 * 0.001 = 0.25
            // 净收入：250 - 0.25 = 249.75
            ...
            // 5.1 价格上涨至 300
            (getPrice as jest.Mock).mockReturnValue(300);
