...
describe('复杂交易场景测试', () => {
    ...
    beforeEach(() => {
        ...
        exchange = new Exchange(wallet, FEE_RATE);
        (getPrice as jest.Mock).mockImplementation(() => 40000);
    });
    ...
    test('现货交易 + 价格波动场景', () => {
        ...
        // 2. 价格上涨到 42000
        (getPrice as jest.Mock).mockImplementation(() => 42000);
        ...
        // 3. 卖出 0.05 BTC @ 42000
        exchange.spotSell(Symbol.BTC_USDT, 0.05);
    ...
    test('挂单 + 价格波动场景', () => {
        // 1. 挂买单 0.1 BTC @ 39000
        const orderId = exchange.placeBuyOrder(Symbol.BTC_USDT, 0.1, 39000);
        ...
        // 2. 价格下跌到 39000，触发买单
        (getPrice as jest.Mock).mockImplementation(() => 39000);
    ...
    test('空单 + 价格波动场景', () => {
        ...
        exchange.openShort(Symbol.BTC_USDT, 0.1, 10);
        ...
        // 2. 价格下跌到 38000
        (getPrice as jest.Mock).mockImplementation(() => 38000);
    ...
    test('混合交易场景', () => {
        ...
        exchange.openShort(Symbol.BTC_USDT, 0.05, 10);
        ...
        // 3. 价格波动到 41000
        (getPrice as jest.Mock).mockImplementation(() => 41000);
        exchange.onPriceUpdate(Symbol.BTC_USDT, 41000);
        ...
        exchange.placeSellOrder(Symbol.BTC_USDT, 42000, 0.05);
