...
interface GridConfig {
    gridCount: number;          // 网格数量（单边）
    gridSpacing: number;        // 网格间距（百分比）
    spotRatio: number;         // 现货资金占比
    contractRatio: number;     // 合约保证金占比
    safeZoneRange: number;     // 安全区范围（百分比）
    warningZoneRange: number;  // 预警区范围（百分比）
    dangerZoneRange: number;   // 危险区范围（百分比）
    warningHedgeRatio: number; // 预警区对冲比例
    dangerHedgeRatio: number;  // 危险区对冲比例
    baseHedgeRatio: number;    // 基础对冲比例
}
...
interface HighFreqConfig {
    enabled: boolean;           // 是否启用高频模式
    minSpread: number;         // 最小价差
    maxHoldTime: number;       // 最大持仓时间（毫秒）
    tradeSize: number;         // 每次交易金额
    maxOpenPositions: number;  // 最大开仓数量
}

interface StepTakeProfitOrder {
    targetPrice: number;
    amount: number;
    orderId?: string;
}
...
export class DynamicHedgeGridStrategy implements Strategy {
    private exchange!: Exchange;  // 使用 ! 表示会在 execute 中初始化
    private config: GridConfig;
    private basePrice: number;
    private gridOrders: GridOrder[];
    private lastRebalanceTime: number;
    private initialTotalValue: number;
    private dropBasePrice: number;
    private dropBuyAmount: number;
    private pumpBasePrice: number;
    private pauseSpotBuying: boolean;
    private pauseSpotSelling: boolean;
    private priceHistory: Map<Symbol, number[]>;
    private tradingPausedUntil: Map<Symbol, { buy: number; sell: number }>;
    private trendData: Map<Symbol, { highs: number[]; lows: number[]; }>;
    private trailingStopPrice: number;
    private highFreqConfig: HighFreqConfig;
    private highFreqPositions: Map<string, {
        entryPrice: number;
        amount: number;
        timestamp: number;
        isBuy: boolean;
    }>;
    private stepTakeProfitOrders: Map<Symbol, StepTakeProfitOrder[]>;
    ...
    constructor(config: Partial<GridConfig> = {}) {  // 移除 exchange 参数
        this.config = {
     ...
     * 初始化策略
     */
    async init(symbol: Symbol): Promise<void> {
        // 1. 获取当前价格作为基准价
        this.basePrice = getPrice(symbol);
     ...
     * 计算网格订单
     */
    private calculateGridOrders(symbol: Symbol): GridOrder[] {
     ...
     * 建立初始对冲仓位
     */
    private async setupInitialHedge(symbol: Symbol): Promise<void> {
        const totalValue = this.exchange.getTotalAssetValue();
     ...
     * 挂单
     */
    private async placeGridOrders(symbol: Symbol): Promise<void> {
     ...
     * 计算当前价格所在区域
     */
    private calculatePriceZone(currentPrice: number): 'safe' | 'warning' | 'danger' {
     ...
     * 获取对冲比例
     */
    private getHedgeRatio(zone: 'safe' | 'warning' | 'danger'): number {
        switch (zone) {
            case 'safe':
     ...
     * 处理价格更新
     */
    onPriceUpdate(symbol: Symbol, price: number): void {
        // 1. 检查是否需要重新平衡（每 4 小时）
        const now = Date.now();
     ...
     * 重新平衡策略
     */
    private async rebalance(symbol: Symbol, currentPrice: number): Promise<void> {
        // 1. 取消所有挂单
     ...
     * 更新对冲仓位
     */
    private async updateHedgePosition(symbol: Symbol, currentPrice: number): Promise<void> {
        // 1. 计算价格区域
     ...
     * 执行策略
     */
    private async executeStrategy(symbol: Symbol, price: number, timestamp: number): Promise<void> {
        try {
            // 1. 更新价格历史
     ...
     * 动态调整对冲仓位
     */
    private async adjustHedgePosition(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 根据波动率调整杠杆
     */
    private async adjustLeverageByVolatility(symbol: Symbol, price: number): Promise<void> {
     ...
     * 根据趋势调整对冲比例
     */
    private async adjustHedgeRatioByTrend(symbol: Symbol, price: number): Promise<void> {
     ...
     * 检查对冲效率
     */
    private async checkHedgeEfficiency(symbol: Symbol, price: number): Promise<void> {
        const position = this.exchange.getShortPosition(symbol);
        if (!position) return;
     ...
     * 计算现货盈亏
     */
    private calculateSpotPnl(symbol: Symbol, currentPrice: number): number {
     ...
     * 计算平均入场价格
     */
    private calculateAverageEntryPrice(symbol: Symbol): number {
        let totalValue = 0;
        let totalAmount = 0;
     ...
     * 检查止盈条件
     */
    private async checkTakeProfit(symbol: Symbol, price: number, timestamp: number): Promise<void> {
        // 1. 网格止盈
     ...
     * 检查网格止盈
     */
    private async checkGridTakeProfit(symbol: Symbol, currentPrice: number): Promise<void> {
     ...
     * 检查趋势止盈
     */
    private async checkTrendTakeProfit(symbol: Symbol, price: number, timestamp: number): Promise<void> {
        // 更新趋势数据
     ...
     * 检查对冲止盈
     */
    private async checkHedgeTakeProfit(symbol: Symbol, price: number): Promise<void> {
        const position = this.exchange.getShortPosition(symbol);
        if (!position) return;
     ...
     * 检查止损条件
     */
    private async checkStopLoss(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 检查网格级止损
     */
    private async checkGridStopLoss(symbol: Symbol, currentPrice: number): Promise<void> {
     ...
     * 检查账户级止损
     */
    private async checkAccountStopLoss(symbol: Symbol, currentPrice: number): Promise<void> {
     ...
     * 检查时间止损
     */
    private async checkTimeStopLoss(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 处理特殊市场情况
     */
    private async handleSpecialMarketConditions(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 处理急速下跌
     */
    private async handleRapidDrop(symbol: Symbol, price: number): Promise<void> {
        // 1. 暂停现货买入
     ...
     * 处理暴力拉升
     */
    private async handleViolentPump(symbol: Symbol, price: number): Promise<void> {
        // 1. 暂停现货卖出
     ...
     * 处理横盘震荡
     */
    private async handleSideways(symbol: Symbol, price: number): Promise<void> {
        // 1. 收缩网格间距
     ...
     * 检查是否处于急速下跌
     */
    private isRapidDrop(priceHistory: number[]): boolean {
     ...
     * 检查是否处于暴力拉升
     */
    private isViolentPump(priceHistory: number[]): boolean {
     ...
     * 检查是否处于横盘震荡
     */
    private isSideways(priceHistory: number[]): boolean {
     ...
     * 更新价格历史
     */
    private updatePriceHistory(symbol: Symbol, price: number): void {
     ...
     * 获取价格历史
     */
    private getPriceHistory(symbol: Symbol): number[] {
     ...
     * 检查交易是否被暂停
     */
    private isTraidingPaused(symbol: Symbol, isBuy: boolean): boolean {
     ...
     * 暂停交易
     */
    private pauseTrading(symbol: Symbol, isBuy: boolean, duration: number): void {
     ...
     * 设置阶梯止盈
     */
    private async setupStepTakeProfit(symbol: Symbol, price: number): Promise<void> {
        ...
        const orders: StepTakeProfitOrder[] = [];
     ...
     * 取消阶梯止盈订单
     */
    private async cancelStepTakeProfitOrders(symbol: Symbol): Promise<void> {
     ...
     * 检查是否连续创新高
     */
    private isConsecutiveHighs(highs: number[]): boolean {
     ...
     * 检查是否连续创新低
     */
    private isConsecutiveLows(lows: number[]): boolean {
     ...
     * 根据趋势调整仓位
     */
    private async adjustPositionForTrend(symbol: Symbol, price: number, isUptrend: boolean): Promise<void> {
        const position = this.exchange.getShortPosition(symbol);
        if (!position) return;
     ...
     * 设置追踪止盈
     */
    private setupTrailingStop(symbol: Symbol, price: number, isUptrend: boolean): void {
     ...
     * 检查是否新的交易日
     */
    private isNewTradingDay(timestamp: number): boolean {
     ...
     * 执行高频交易
     */
    private async executeHighFreqTrading(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 清理超时仓位
     */
    private async cleanupStalePositions(symbol: Symbol, timestamp: number): Promise<void> {
     ...
     * 开立高频仓位
     */
    private async openHighFreqPosition(symbol: Symbol, price: number, timestamp: number): Promise<void> {
     ...
     * 计算短期趋势
     */
    private calculateShortTermTrend(symbol: Symbol): number {
     ...
     * 添加风险控制措施
     */
    private async checkRiskControl(symbol: Symbol, price: number): Promise<void> {
        // 1. 检查杠杆率
     ...
     * 检查杠杆率风险
     */
    private async checkLeverageRisk(symbol: Symbol, price: number): Promise<void> {
     ...
     * 检查集中度风险
     */
    private async checkConcentrationRisk(symbol: Symbol): Promise<void> {
        const MAX_POSITION_RATIO = 0.3; // 单个资产最大占比 30%
        
        const totalValue = this.exchange.getTotalAssetValue();
        const symbolValue = this.exchange.getBalance(symbol) * getPrice(symbol);
     ...
     * 检查流动性风险
     */
    private async checkLiquidityRisk(symbol: Symbol): Promise<void> {
     ...
     * 获取流动性指标
     */
    private async getLiquidityMetrics(symbol: Symbol): Promise<LiquidityMetrics> {
        // TODO: 从交易所获取实际数据
        return {
     ...
     * 分析流动性指标
     */
    private analyzeLiquidityMetrics(metrics: LiquidityMetrics): boolean {
     ...
     * 调整低流动性应对策略
     */
    private async adjustForLowLiquidity(symbol: Symbol, metrics: LiquidityMetrics): Promise<void> {
        // 1. 减小交易规模
        const sizeAdjustment = Math.min(
            metrics.orderBookDepth / 1000000,  // 基于订单簿深度
     ...
     * 根据流动性调整止损
     */
    private async adjustStopLossForLiquidity(symbol: Symbol, metrics: LiquidityMetrics): Promise<void> {
        // 基于价差调整止损比例
     ...
     * 动态调整止损
     */
    private async adjustDynamicStopLoss(symbol: Symbol, price: number): Promise<void> {
        const position = this.exchange.getShortPosition(symbol);
        if (!position) return;
     ...
     * 平掉空头仓位
     */
    private async closeShortPosition(symbol: Symbol): Promise<void> {
        await this.exchange.closeShort(symbol);
        this.trailingStopPrice = 0;
    }
     ...
     * 检查波动率风险
     */
    private async checkVolatilityRisk(symbol: Symbol): Promise<void> {
     ...
     * 更新对冲效率
     */
    private updateHedgeEfficiency(): void {
     ...
     * 优化对冲效率
     */
    private async optimizeHedgeEfficiency(symbol: Symbol, price: number): Promise<void> {
     ...
     * 计算最优杠杆
     */
    private calculateOptimalLeverage(volatility: number, hedgeEfficiency: number): number {
        // 基于波动率和对冲效率计算最优杠杆
